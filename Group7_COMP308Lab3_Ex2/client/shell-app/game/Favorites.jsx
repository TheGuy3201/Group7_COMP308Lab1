...
const REMOVE_FAVORITE_GAME = gql`
  mutation RemoveFavouriteGame($userId: ID!, $gameId: ID!) {
    removeFavouriteGame(userId: $userId, gameId: $gameId) {
      userId
      favouriteGames {
        gameId
        title
...
export default function Favorites() {
  ...
  const favoriteGameIds = new Set(favoritesData?.user?.favouriteGames?.map((g) => g.gameId) || []);
  const allGames = gamesData?.games || [];
  const favoriteGames = favoritesData?.user?.favouriteGames || [];
  const availableGames = allGames.filter((game) => !favoriteGameIds.has(game.gameId));

  const handleAddToFavorites = (gameId) => {
    addFavorite({ variables: { userId, gameId } });
    setAddDialogOpen(false);
  };

  const handleRemoveFromFavorites = (gameId) => {
    removeFavorite({ variables: { userId, gameId } });
  };
